...
assert.equal(zaloPartCountForSize(89 * MiB), 1);
assert.equal(zaloPartCountForSize(90 * MiB), 1);
assert.equal(zaloPartCountForSize(90 * MiB + 1), 2);
assert.equal(zaloPartCountForSize(180 * MiB), 3);
console.log('social video transfer-limit guards ok');
