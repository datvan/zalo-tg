...
const zaloHandlerSource = readFileSync(new URL('../src/zalo/handler.ts', import.meta.url), 'utf8');
assert.match(zaloHandlerSource, /prepareTelegramVideoPaths/);
assert.match(zaloHandlerSource, /sendVideo/);
assert.match(telegramHandlerSource, /prepareZaloVideoPaths/);
assert.match(telegramHandlerSource, /prepareTelegramVideoPaths/);
assert.match(telegramHandlerSource, /TG->TG.*Sent part/);
...
selfVideo.rememberSelfVideoMessage({ message: { msgId: 103 } });
assert.equal(selfVideo.consumeSelfVideoMessage('103'), true);
selfVideo.rememberSelfVideoUrl('https://www.facebook.com/reel/123/?x=1');
assert.equal(selfVideo.consumeSelfVideoUrl('https://www.facebook.com/reel/123/?x=1'), true);
assert.equal(selfVideo.consumeSelfVideoUrl('https://www.facebook.com/reel/123/?x=1'), false);
